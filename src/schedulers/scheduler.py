...
class QualityCheckScheduler:
            ...
            try:
                ...
                if source == "hackernews":
                    ...
                    if self.ingestor.reddit:
                        df = self.ingestor.reddit.fetch_data(subreddit_name="technology", limit=50)
                    else:
                        logger.warning("RedditIngestor not initialized (Check credentials)")
                        continue
                ...
                logger.info(f"Quality Check for {source}: Score {results['overall_score']} ({results['status']})")
    ...
    def start(self):
        ...
        logger.info(f"Scheduler started: Running every {settings.CHECK_INTERVAL_HOURS} hour(s)")
